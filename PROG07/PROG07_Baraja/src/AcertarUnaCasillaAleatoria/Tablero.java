...
public class Tablero implements Serializable{
    ...
    // ATRIBUTOS
    private int numFilas; // Número de casillas del ancho/alto del tablero
    ...
    public Tablero(int numFilas) {
        ...
        arrayCasillas[(int) (Math.random() * numFilas)][(int) (Math.random() * numFilas)].setPremio(new Premio());
    }
    ...
    public void mostrarTablero() {
        for (int i = 0; i < numFilas; i++) {
            for (int j = 0; j < numFilas; j++) {
                if (arrayCasillas[i][j].isElegida() && arrayCasillas[i][j].getPremio()!=null) {
                    System.out.print(" X ");
                } else if (arrayCasillas[i][j].isElegida() && arrayCasillas[i][j].getPremio()==null) {
                    System.out.print(" O ");
                } else {
                    System.out.print(" - ");
                }
