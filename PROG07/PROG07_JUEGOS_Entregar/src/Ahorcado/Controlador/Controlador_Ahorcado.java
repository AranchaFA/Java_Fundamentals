...
public class Controlador_Ahorcado {
    ...
    public Controlador_Ahorcado(String palabraAAcertar, String nombreJugador, int maxFallos) {
        ...
        // Creamos el objeto acierto pasando la longitud de la palabra
        acierto = new Acierto(this.palabraAAcertar.length());
    }
    ...
    public boolean realizarUnaJugada(char letra) {
        ...
        // grabamos en la misma posición en que se encuentra en el array de acierto
        for (int i = 0; i < palabraAAcertar.length(); i++) {
            if (palabraAAcertar.charAt(i) == letra) {
                acierto.grabarLetra(i, letra);
                existe = true;
            }
        }
        ...
        palabraAcertada=acierto.comprobarPalabraAcertada();
    ...
    public void echarUnaPartida() {
        ...
        do {
            // Mostramos el estado del array de acierto
            System.out.print("\nPalabra a acertar:  ");
            acierto.mostrarArray();
    ...
    public void grabarResultados() {
        String acertada;
        if (acierto.comprobarPalabraAcertada()) {
